...
plot_patient_timeline <- function(processed, pat_id) {
  ...
  mrd_plot <- ggplot2::ggplot() +
    ...
    ggplot2::geom_line(data = d$mrd |> dplyr::filter(!is.na(Mutation)) |> dplyr::group_by(Mutation) |> dplyr::filter(dplyr::n() > 1) |> dplyr::ungroup(),
    ...
    geomtextpath::geom_textvline(data = d$general_info |> dplyr::filter(outcome == "Relapse"), ggplot2::aes(xintercept = rel_term_dat, label = "Relapse")) +
...
draw_clinical_course <- function(
  ...
  purrr::walk(unique(processed$general_info$patno),
