...
draw_clinical_course <- function(
  ...
  plot_patient_timeline <- function(processed, pat_id) {
    ...
    combined_legends <- cowplot::plot_grid(mrd_legend, agvhd_legend_grob, cgvhd_legend_grob, ncol=1, align="v")
    
    # Final combined plot
    final_plot <- cowplot::plot_grid(combined_plots, combined_legends, ncol=2, rel_widths=c(4,1), align="v")
    return(final_plot)
  }
